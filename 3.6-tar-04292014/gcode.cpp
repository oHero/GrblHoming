...
/// LETARTARE :
				msg = QString(tr("XYUV-Rate Limit FIX: [%1]=>[%2]")).arg(inputLine).arg(line);
                list.append(line);
            }
            else
            {
...
/// LETARTARE
    QString unit("") ;
			millimeters == true ? unit = "(mm)"
								: unit = "(in)";
			// 4 axes
			if (controlParams.useFourAxis) {
				QChar four(controlParams.fourthAxisType);
				// rotary axis
				if (four == FOURTH_AXIS_A || four == FOURTH_AXIS_B || four == FOURTH_AXIS_C )
				{
					QString axis(" / ");
					unit.append(axis).append(four).append(" (deg.)");
				}
			}
			// emit unit
			emit setUnitsMachine(tr(qPrintable(unit)));
			emit setUnitsWork(tr(qPrintable(unit)));
}
